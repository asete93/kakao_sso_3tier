...
export function middleware(req: NextRequest) {
  ...
  if(pathname.startsWith('/login') && req.cookies.get('_xjd')?.value){
  ...
  if(pathname.startsWith('/signup') && !req.cookies.get('_xjd')?.value){
    return NextResponse.next();
  }

  if(pathname=="/" && !req.cookies.get('_xjd')?.value ){
    return NextResponse.redirect(new URL('/login', req.url));
  }

  // 쿠키에서 loginUser 추출.
  const loginUser = req.cookies.get('_xjd')?.value;
